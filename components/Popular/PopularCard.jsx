...
useEffect(()=>{
  ...
  .then(res=>{
    ...
    return (
        <div className={`flex w-full py-1 border-b border-orange-400 flex-row md:h-max min-h-full h-max bg-base-100 sm:h-fit  shadow-lg`}>
        <Image
          className="block md:w-44 mr-1 w-12 sm:w-24 border-2 md:border-4 border-blue-400 flex-none bg-cover md:h-40 h-12 sm:h-24 object-fill"
        ...
        <div className="text-sm hidden md:block lg:hidden leading-5">{post?.description?.split(' ').slice(0,22).join(' ')}...</div>
        <div className="text-xs block md:hidden leading-5">{post?.description?.split(' ').slice(0,22).join(' ')}...</div>
         <div>
